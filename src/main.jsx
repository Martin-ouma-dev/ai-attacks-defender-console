...
function App() {
  ...
  return (
    <div className="min-h-screen bg-ink text-slate">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-20 grid grid-cols-4 border-t border-electric/25 bg-panel/95 px-1 py-2 backdrop-blur-xl md:inset-x-auto md:bottom-0 md:left-0 md:top-[72px] md:flex md:w-64 md:flex-col md:justify-start md:gap-2 md:border-r md:border-t-0 md:px-5 md:py-8">
        ...
        {["Dashboard", "Agents", "Logs", "Settings"].map((item) => <button key={item} onClick={() => setActive(item)} className={`min-w-0 rounded-xl px-1 py-2 text-center font-mono text-[8px] uppercase tracking-[.08em] transition sm:px-2 sm:text-[9px] sm:tracking-widest md:flex md:items-center md:gap-3 md:px-4 md:py-3 md:text-left md:text-[10px] ${active === item ? "bg-electric/10 text-cyan shadow-[inset_0_2px_0_#38bdf8] md:shadow-[inset_3px_0_0_#38bdf8]" : "text-slate/70 hover:bg-white/[.04] hover:text-cyan"}`}><span className="block text-center text-sm sm:text-base md:w-5">{item === "Dashboard" ? "◉" : item === "Agents" ? "◈" : item === "Logs" ? "▤" : "⚙"}</span>{item}</button>)}
